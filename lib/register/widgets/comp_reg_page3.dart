        ...
        child: Column(
          ...
          children: [
            ...
            StreamBuilder<List<ZoneModel>>(
              stream: controller.zonesStream,
              initialData: controller.zonesI ?? [],
              builder: (BuildContext context,
                  AsyncSnapshot<List<ZoneModel>> snapshot) {
                return DropdownSearch<ZoneModel>(
                  ...
                  popupProps: PopupProps.menu(
                      ...
                      fit: FlexFit.loose,
                      showSearchBox: true),
                  compareFn: (i1, i2) => i1.zoneCode == i2.zoneCode,
                  dropdownDecoratorProps: DropDownDecoratorProps(
                      dropdownSearchDecoration: InputDecoration(
                          labelText: "Zona",
                          contentPadding:
                          ...
                          border: OutlineInputBorder(
                              borderRadius: BorderRadius.circular(radiusValue),
                              borderSide: BorderSide.none))),
                  selectedItem: controller.companyData.zone,
                  items: snapshot.data!,
                  validator: (value) {
                    if (value == null) {
                      return "Campo requerido";
                  ...
                  onChanged: (ZoneModel? value) async {
                    if (controller.companyData.zone?.zoneCode !=
                        value?.zoneCode) {
                      controller.companyData.zone = value;

                      await controller.reloadSZones();
                    }
                  },
                );
              },
            ).paddingSymmetric(horizontal: 16, vertical: 6),
            StreamBuilder<List<SubZoneModel>>(
              stream: controller.sZonesStream,
              initialData: controller.sZonesI ?? [],
              builder: (BuildContext context,
                  AsyncSnapshot<List<SubZoneModel>> snapshot) {
                return DropdownSearch<SubZoneModel>(
                  ...
                  popupProps: PopupProps.menu(
                      emptyBuilder: (context, String? string) {
                        return const Center(
                          child: Text("No se encontraron barrios"),
                        );
                      },
                      menuProps: MenuProps(
                          borderRadius: BorderRadius.circular(radiusValue2),
                      ...
                      fit: FlexFit.loose,
                      showSearchBox: true),
                  compareFn: (i1, i2) => i1.subzoneCode == i2.subzoneCode,
                  dropdownDecoratorProps: DropDownDecoratorProps(
                      dropdownSearchDecoration: InputDecoration(
                          labelText: "Barrio",
                          contentPadding:
                          ...
                          border: OutlineInputBorder(
                              borderRadius: BorderRadius.circular(radiusValue),
                              borderSide: BorderSide.none))),
                  selectedItem: controller.companyData.sZone,
                  items: snapshot.data!,
                  // validator: (value) {
                  //   if (value == null) {
                  //     return "Campo requerido";
                  ...
                  //   return null;
                  // },
                  onChanged: (SubZoneModel? value) async {
                    // setState(() {
                    controller.companyData.sZone = value;
                    // });
                  },
                );
              },
